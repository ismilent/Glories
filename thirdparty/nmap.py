...
def parse_callback_result(scan_result):
    '''
     address, port, service, state, protocol, product, product_version
     product_extrainfo, scripts_results
    '''
    host = scan_result.keys()
    print host
...
############################################################################
def __scan_progressive_new__(self, hosts, ports, arguments, callback, sudo):
    try:
...
class PortScannerAsync(object):
    ...
    def scan(self, hosts='127.0.0.1', ports=None, arguments='-sV', callback=None, sudo=False):
        ...
        '''
        self._process = Process(
            target = __scan_progressive_new__,
            args = (self, hosts, ports, arguments, callback, sudo)
        )
        '''
